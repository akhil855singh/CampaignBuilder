...
const DecisionsModalView = ({ popupType }: Props) => {
  ...
  const CustomFormField: React.FC<CustomFormFieldProps> = ({ label, placeholder, labelColor = "black", icon }) => <FormControl mt={4}>
    <FormLabel color="black">
      {label} <Text as="span" color={labelColor}>{icon}</Text>
    </FormLabel>
  ...
  const ExpandableTextArea: React.FC<ExpandableTextAreaProps> = ({ label, placeholder }) => {
    return (
      <FormControl mt={4}>
  ...
  const SingleSelect: React.FC<SingleSelectProps> = ({ label, options, placeholder = "Select an option", onChange }) => {
    ...
    return (
      <FormControl>
        <FormLabel display="flex" alignItems="center" fontWeight="bold" margin={2}>
          {label}
          <Icon as={InfoIcon} ml={1} color="gray.400" boxSize={4} /> {/* Info icon */}
        </FormLabel>

        <InputGroup>
          <Select value={selectedValue} onChange={handleChange} placeholder={placeholder}>
            {options.map((option) => (
    ...
    <Flex position="relative" zIndex={20} align="flex-start" justify="center" minH="100vh" bg="rgba(0, 0, 0, 0.4)">
      <Box bg="white" p={6} rounded="md" boxShadow="lg" w="600px">
        ...
        {(popupType === DecisionsTypes.SENDS_A_APP_PUSH_MESSAGE ||
          ...
          popupType === DecisionsTypes.SENDS_A_WHATS_APP_MESSAGE)
          &&
          <CustomFormField label="Pattern the reply should match" placeholder="" labelColor="gray.300" icon="❓"></CustomFormField>
        }
        ...
        {popupType === DecisionsTypes.VISITS_A_PAGE && <CustomFormField label="URL" placeholder="" labelColor="gray.300" icon="❓"></CustomFormField>}
        {popupType === DecisionsTypes.VISITS_A_PAGE && <CustomFormField label="Referrer" placeholder="" labelColor="gray.300" icon="❓"></CustomFormField>}
        ...
        {popupType === ActionsTypes.ADD_DO_NOT_CONTACT &&
          <ExpandableTextArea label="Reason" placeholder="Enter your reason here..." />}

        {popupType === ActionsTypes.ADD_TO_COMPANY_SCORE &&
          <CustomFormField label="Add to company's score" placeholder="" labelColor="red" icon="*"></CustomFormField>}

        {popupType === ActionsTypes.ADD_TO_COMPANY_ACTION &&
          <CustomFormField label="Companies" placeholder="" labelColor="red" icon="*"></CustomFormField>}
        {popupType === ActionsTypes.ADJUST_CONTACT_POINTS &&
          <CustomFormField label="Points (+/-)" placeholder="" labelColor="red" icon="*"></CustomFormField>}
