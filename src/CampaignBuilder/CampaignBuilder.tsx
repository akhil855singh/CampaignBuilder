...
function CampaignBuilder() {
    ...
    nodeType = useMemo(
        () => ({
            text: (props) => (
                <TextNode { ...props } selectionViewActive={ (value) => setModalActive(value) } copyNode={ (node) => setCopyNode(node) } updateNode={ (id) => updateNodeData(id) } setNodes={ setNodes } handleHandleClick={ handleHandleClick } />
            ),
        }), [setNodes]);
